...
# 声母
__shengmu = set(['b','p','m','f','d','t','n','l','g','k','h','j','q','x','zh','ch','sh','r','z','c','s','y','w'])
# 韵母
...
def get_yunmu(one_py):
    if len(one_py) == 0:
        ...
        if is_yunmu(one_py[:2]):
            return one_py[:2]
    else:
        if is_yunmu(one_py[:3]):
            return one_py[:3]
        elif is_yunmu(one_py[:2]):
            return one_py[:2]
        elif is_yunmu(one_py[:1]):
            return one_py[:1]
